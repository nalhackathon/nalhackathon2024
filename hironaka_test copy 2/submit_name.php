...
<head>
    ...
    <script>
        ...
        function openNewWindow(username) {
            window.open('new_window.php?username=' + encodeURIComponent(username), '_blank', 'width=600,height=400');
        }
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <?php if ($row['username'] === $username && count(explode(',', $row['discard'])) >= 5): ?>
                            <button onclick="openNewWindow('<?= htmlspecialchars($row['username']) ?>')">提出</button>
                        <?php endif; ?>
                    </td>
                    <td>
