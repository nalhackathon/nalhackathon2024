...
<body>
    ...
    <div id="container">
        ...
        <div id="card-table">
            ...
            // GETパラメータからユーザー名を取得し、セッションに保存
            if (isset($_GET['username'])) {
                $_SESSION['selected_user'] = $_GET['username'];
            }

            // セッションからユーザー名を取得
            $username = $_SESSION['selected_user'] ?? null;

            if ($username) {
                // データベースから特定のユーザーの username, card, reason2 を取得して表示
                $sql = "SELECT username, card, reason2 FROM test WHERE username = :username";
                $stmt = $db->prepare($sql);
